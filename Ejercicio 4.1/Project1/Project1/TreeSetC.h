...
public:
	...
	int count_interval(T const& lower, T const& upper) const {
		return count_interval_helper(ra, lower, upper);
	}

	int count_interval_helper(Nodo* node, T lower, T upper) const {
		if (node == nullptr)
		{
			return 0;
		}
		...
		if (node->elem >= lower && node->elem <= upper)
			count++;

		if (node->elem > lower)
			count += count_interval_helper(node->iz, lower, upper);

		if (node->elem < upper)
			count += count_interval_helper(node->dr, lower, upper);
	...
	public:
        ...
        /** Conversor de Iterator a ConstIterator  */
        ConstIterator(const Iterator& it){
            this->act = it.act;
            this->ascendientes = it.ascendientes;
        }

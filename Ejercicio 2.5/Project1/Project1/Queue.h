...
public:
	...
	void IntroducirCola(Queue<T>& colegas, const T& pringao)
	{
		Nodo* actual = prim;
		Nodo* aux = nullptr;

		Nodo* continuacion = nullptr; 
		Nodo* primercolega = colegas.prim;

		int contador = colegas.size();
		bool encontrado = false;

		while (actual != nullptr && !encontrado) {
			if (actual->elem == pringao) {

				aux = actual; //nodo encontrado
		...
		//Si aux = nullptr eso significa que no se ha encontrado al pringao
		if (aux != nullptr)
		{
			aux->sig = primercolega;
			do
			{
				aux = aux->sig;
			} while (contador-- - 1 > 0);
			aux->sig = continuacion;

		}

		//Eliminamos la cola colega  

/**
 * Conjuntos genéricos representados por un array dinámico ordenado y sin repeticiones
 * (c) Miguel Gómez Zamalloa
...
void Set<T>::diff(const Set<T>& set)
{
	...
	{
		if (this->elems[i] == set.elems[contador])
		{
		...
		else if (this->elems[i] > set.elems[contador])
		{
			contador++;
			i--;
		}

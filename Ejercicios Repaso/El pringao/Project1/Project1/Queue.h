...
public:
	...
	void colar(Queue<int>& bullys, int pringao)
	{
		Nodo* actual = prim;
		Nodo* aux = nullptr;
		bool pringao_encontrado = false;

		while (actual != nullptr && !pringao_encontrado)
		{
			if (actual->elem == pringao)
			{
				pringao_encontrado = true;
			}
			else
			{
				actual = actual->sig;
		...
		if (pringao_encontrado)
		{
			aux = actual->sig;
			actual->sig = bullys.prim;
			bullys.ult->sig = aux;

			bullys.prim = nullptr;
			bullys.ult = bullys.prim;

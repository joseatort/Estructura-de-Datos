...
public:
  ...
  void zip(Dequeue<int>& other)
  {
      Nodo* actual_this = prim->sig;
      Nodo* aux_this = actual_this->sig;

      Nodo* actual_other = other.prim->sig;
      Nodo* aux_other = actual_other->sig;

      bool fin_this = false;

      while (actual_other != other.prim)
      {

          if (actual_this->sig == prim && actual_other->sig != other.prim)
              fin_this = true;


          actual_this->sig = actual_other;
          actual_other->ant = actual_this;



          if (fin_this == true)
          {
              other.prim->ant->sig = this->prim;
              break;
          }



          actual_other->sig = aux_this;
          aux_this->ant = actual_other;

          actual_this = actual_other->sig;
          actual_other = aux_other;

          aux_this = aux_this->sig;
          aux_other = aux_other->sig;
      }

      other.prim->sig = other.prim;
      other.prim->ant = other.prim;

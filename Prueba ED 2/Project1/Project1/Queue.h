...
public:
    ...
    void cuela(const T& a, const T& b)
	{
		...
		Nodo* actual = prim;			//Inicializamos
		Nodo* aux_paraA = nullptr;		//posicion A 

		Nodo* aux = nullptr;			//Variables auxiliares
		Nodo* continuacion = nullptr;
		Nodo* prueba = nullptr;
		Nodo* anteriorB = nullptr;


		Nodo* aux_paraB = nullptr;		//posicion B

		bool encontrado = false;		//Boleanos para cortar bucles
		...
		while (actual->sig != nullptr)
		{
			...
			if (actual->elem == a && encontrado == false)
			{
				aux_paraA = actual;
				aux = aux_paraA->sig;
				continuacion = aux_paraA->sig;
				encontrado = true;
			}
			else if((actual->sig->elem == b && encontrado2 == false) && encontrado == true)
			{
				anteriorB = actual;
				encontrado2 = true;
		...
		if (aux_paraB != nullptr && aux_paraA != nullptr && aux_paraA->sig != aux_paraB)
		{
			aux_paraA->sig = aux_paraB;
			aux_paraB->sig = continuacion;
			anteriorB->sig = prueba;
